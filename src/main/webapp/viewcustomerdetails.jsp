...
<%-- <%
   ...
   String jdbcUrl = "jdbc:mysql://localhost:3306/parlour";
   ...
   try {
      ...
      // Execute query to retrieve records matching the search criteria
      String sql = "SELECT * FROM reg WHERE name LIKE ?";
      PreparedStatement statement = connection.prepareStatement(sql);
      statement.setString(1, "%" + searchQuery + "%");
      ResultSet resultSet = statement.executeQuery();
      
      // Display matching records to the user
      while (resultSet.next()) {
         out.println(resultSet.getString("name"));
         // ...display other columns as required
      }
...
rs = stmt.executeQuery("SELECT COUNT(*) AS count FROM reg");
if(rs.next()) {
 total = rs.getInt("count");
}

//Get rows for current page
rs = stmt.executeQuery("SELECT * FROM reg LIMIT " + start + "," + limit);
